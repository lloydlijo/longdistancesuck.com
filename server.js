...
io.on('connection', (socket) => {
  ...
  socket.on('login', (password) => {
    ...
    if (username) {
      ...
      // Send current video state (filename only, no URL)
  ...
  // --- WebRTC: Only caller creates offer ---
  socket.on('start-call', () => {
    // Notify other user of incoming call; they should NOT create an offer
  ...
  // --- Movie: filename sync (no file transfer) ---
  socket.on('video-file-loaded', (fileName) => {
    currentVideoFileName = fileName;
    currentVideoTime = 0;
    isPlaying = false;
    // Tell other user to load their own copy of the same file
    socket.broadcast.emit('video-file-request', fileName);
  });

  // --- Movie tab open: redirect other user ---
  socket.on('open-movie-tab', () => {
    socket.broadcast.emit('open-movie-tab');
  });
  ...
  socket.on('video-skip', (data) => {
    // data: { time, delta } - forward/backward skip
    currentVideoTime = data.time;
    socket.broadcast.emit('video-seek', data.time);
  });
